...
def authorize(
    ...
    try:
        intent = client.PaymentIntent.create(
            ...
            shipping=shipping,
            return_url='https://sashsash.free.beeceptor.com/payment-redirect/',
        ...
        # Now that we have the intent we can define what our response will it be
        # How could we define it before? We need to see if intent.status == requires_action
        kind = TransactionKind.ACTION_TO_CONFIRM if intent.status == "requires_action" or intent.status == "requires_source_action" else TransactionKind.CAPTURE

    except stripe.error.StripeError as exc:
        response = _error_response(kind=kind, exc=exc, payment_info=payment_information)
    else:
        success = intent.status in ("succeeded", "requires_capture", "requires_action","requires_source_action")
...
def capture(payment_information: PaymentData, config: GatewayConfig) -> GatewayResponse:
    ...
    try:
        ...
        response = _success_response(
            intent=intent,
            kind=TransactionKind.CAPTURE,
            success=capture.status in ("succeeded", "requires_action", "requires_source_action"),
        )
        response = fill_card_details(intent, response)
    return response
...
def confirm(payment_information: PaymentData, config: GatewayConfig) -> GatewayResponse:
    ...
    try:
        intent = client.PaymentIntent.retrieve(id=payment_information.token)
        intent.confirm(return_url='https://sashsash.free.beeceptor.com/payment-redirect/')
    except stripe.error.StripeError as exc:
        response = _error_response(
        ...
        if (config.auto_capture and intent.status == "succeeded"):
            check_kind = TransactionKind.CAPTURE
        else:
            check_kind = TransactionKind.CAPTURE if intent.status == "succeeded" else TransactionKind.PENDING
        response = _success_response(
            intent=intent,
            kind=check_kind,
            success=intent.status == "succeeded" or intent.status == "requires_capture",
        )
...
def _success_response(
    ...
    return GatewayResponse(
        is_success=success,
        action_required=intent.status == "requires_source_action" or intent.status == "requires_action",
        action_required_data=intent.next_action or intent.next_source_action or [],
